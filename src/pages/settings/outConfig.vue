...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getOutputResolutionRatio() {
        htp.get(urls.getOutputResolutionRatio).then(response => {
          if(response.status === 10000000) {
            this.selectedResolution = response.data;
      ...
      //获取输出画质
      getOutputQuality() {
        htp.get(urls.getOutputQuality).then(res => {
          console.log(res);
        }).catch(error => {
          Toast(error);
        });
      ...
      saveOut() {
        htp.post(urls.setOutputResolutionRatio, {
...
<style scoped lang="less">
  .setting_cont {
      ...
      .vx-progress {
          border-bottom: 1px #4f5054 solid;
