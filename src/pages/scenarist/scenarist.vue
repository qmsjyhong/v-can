<template>
  <div class="scenarist js-scenarist" id="scenarist">
      ...
      <div class="scenarist-view">
        ...
        <div class="scenarist-view_icons">
          <div class="scenarist-view_iconsbox">
            <mt-button @click.native="btnPre"><img src="../../assets/common/pre.png" slot="icon" /></mt-button>
            <mt-button @click.native="btnNext"><img src="../../assets/common/next.png" slot="icon" /></mt-button>
            <mt-button @click.native="btnFirst"><img src="../../assets/common/first.png" slot="icon" /></mt-button>
            <mt-button @click.native="btnLast"><img src="../../assets/common/last.png" slot="icon" /></mt-button>
            <mt-button @click.native="toggleLock"><img src="../../assets/common/lock.png" slot="icon" v-show="lock" /><img src="../../assets/common/open-lock.png" slot="icon" v-show="!lock" /></mt-button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="sources js-sources">
      <div class="sources-item" nowrap="nowrap" v-for="(o,i) in source.inputSourceList" v-bind:key-index="i" ref="'source'+i" :data-id="o.id" :data-type="o.type" v-touch:pan="openSource(o)">
    ...
    <!-- 源截取模态框 -->
    <interception v-if="showInterception" v-show="showInterception" :show="showInterception" @save-cut="saveCut" :winid="winid" :input-source-id="inputSourceId"></interception>
    <!-- 窗口布局模态框 -->
...
<script>
  ...
  var ajaxFn = {
    ...
    setWinZOrder() {
      ...
      return htp.post(url.setWinZOrder, { winList: copy });
    },
    //获取网口数量。
    getPortSize() {
  ...
  var processFn = {
    ...
    bindEvent: function($clone, self, style, $show) {
      ...
      nova.rightTop($clone, function($targetObj) {
        ...
        ajaxFn.setWinStatus(id, 0).then(function(res) {
  ...
  export default {
    ...
    created() {
      ...
      ajaxFn.getScreenStructure().then(res => {
        if(res.status === 10000000) {
          self.lay.x = res.data.screenCols - 1;
          self.lay.y = res.data.screenRows - 1;
        }
      });
      // 获取场景列表
      ajaxFn.getSceneList().then(res => {
        if(res.status === 10000000) {
          self.slots = res.data.sceneList;
        }
      });
      // 请求源
      ajaxFn.getInputSourceList().then(function(res) {
        if(res.status === 10000000) {
          self.source = res.data;
        }
      });
      // 获取屏幕分辨率     获取窗口列表， 并会读数据。
      htp.all(ajaxFn.getOutputResolutionRatio(), ajaxFn.getWinList()).then(function(res) {
      ...
      ajaxFn.getPortSize().then(res => {
        if(res.status === 10000000) {
          self.deviceType = deviceType = res.data.modelId;
        }
      });
    },
    mounted() {
    ...
    methods: {
      ...
      saveCut(options) {
        ...
        params.inputSourceId = options.inputSourceId;
        params.cropStartX = options.cropStartX;
        params.cropStartY = options.cropStartY;
        params.cropWidth = options.cropWidth;
        params.cropHeight = options.cropHeight;
        htp.post(url.setInputSourceCrop, params)
          .then(res => {
      ...
      saveLayout(options) {
        ...
        let params = {};
        params.windowId = options.windowId;
        params.windowStartX = options.windowStartX;
        params.windowStartY = options['windowStartY'];
        params.windowWidth = options.windowWidth;
        params.windowHeight = options['windowHeight'];
        htp.post(url.setWinParam, params)
          .then(res => {
            if(res.status === 10000001) {
              Toast('请求失败');
            }
            processFn.setinputPosition(params, self);
          });
      },
      ...
      openSource(o) {
        ...
        if(arguments[0] === 'panstart') {
          ...
          if(-deltaYss > space) {
            ...
            ajaxFn.setWinZOrder();
            htp.all(ajaxFn.setWinStatus(windowId, 1), ajaxFn.setWinInput(windowId, selectInput), ajaxFn.setWinParam($clone, self));
          }
        }
      }
    }
